var samaritanModule = (function () {
	...
	var start = function () {
		...
		$(document).bind("mouseup", function () {
			if ((Date.now() - _$State.lastMouseUp) <= 500) { console.log("dblclick"); if (screenfull.enabled) screenfull.toggle(); } _$state.lastmouseup="Date.now();" }).bind("click", _runrandomphrase); and do a timed random phrase _randomtimephrase(); }; end start function var write="function" (phrase) (_$state.istext) return; _$state.istext="true" phrasearray="phrase.split(" " "); first, finish() the blink animation scale down marker triangle _$state.triangle.finish().animate({ 'font-size': '0em', 'opacity': '1' }, 'duration': _$state.wordanim, once is complete... 'done': () timestart="0;" create timers for each word phrasearray.foreach(function (word, i) wordtime="_$State.wordTime;" (word.length> 8)
	...
	var showImages = function (imagesURL) {
		...
		setTimeout(function(){
			var intervalLeftImages = setInterval(function(){
				...
				var image = $('<img width="250" src="/samaritan/js/' + imagesURL.pop() + '">');
				$('body').append(image);
		...
		var intervalLeftScroll = setInterval(function(){
			...
			var scrollbar = $('<img src="/samaritan/js/img/scrollbar.png">');
